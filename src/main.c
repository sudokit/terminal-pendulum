...
int main(int argc, char *argv[])
{
    ...
    while (getch() == ERR)
    {
        ...
        // draw_line((vec2){80, 0.5}, (vec2){90, 5});
        mvprintw(0, 0, "origin> x: %f y: %f", origin[0], origin[1]);
        mvprintw(1, 0, "rope end> x: %f y: %f", pos[0], pos[1]);
        mvprintw(2, 0, "screen center> x: %f y: %f", max[0] * 0.5, max[1] * 0.5);
        refresh();

        int currentTime = time(NULL);
        if (nextFrame > currentTime)
        {
            napms(nextFrame - currentTime);
        }
